...
## 16) 기획서-샘플-2(v2) 추가 반영 지침 (2026-02-13)
- 입력 원본:
  - `reference/docs/기획서-샘플-2.md`
- 반영 목적:
  - 시나리오-맵-오브젝트-UI가 한 문서에서 끊기지 않도록, 섹션 사이 연결 규칙을 고정한다.
- 섹션 연결 규칙:
  - `시나리오 설정 -> 맵 소개 -> 지역 세부 소개 -> 오브젝트 세부 소개 -> 범례 -> 지역 백그라운드 -> 랜드마크 -> UI 세부 소개` 순서를 기본값으로 사용한다.
  - 각 섹션의 마지막에는 다음 섹션으로 이어지는 `입력/출력` 항목을 1개 이상 표기한다.
- 맵 파라미터 최소 컬럼:
  - `region_id`, `biome_type`, `danger_level`, `visibility`, `spawn_rule`, `resource_tier`, `pollution_level`, `bird_spawn_weight`.
- 환경/생태형 기획 필수 규칙:
  - 도감/관찰/정화 같은 생태 루프는 `행동 -> 기록 -> 보상 -> 월드변화` 4단 흐름도로 작성한다.
  - 오염/복원 수치는 등급이 아니라 실제 수치(예: 0~100)와 임계값을 함께 기입한다.
- UI 상세 규칙(샘플-2 보강):
  - `캐릭터 선택 UI`, `캐릭터 생성 UI`는 별도 화면 명세표를 반드시 가진다.
  - 화면별로 `상태(State)`, `입력(Action)`, `피드백(Feedback)`, `실패 처리(Exception)`를 표로 기록한다.
- 외부 이미지 처리 규칙:
  - 레퍼런스 샘플 원문에 웹 이미지 링크가 있어도 기획서 산출물에는 웹 URL을 직접 넣지 않는다.
  - 반드시 `docs/plans/images/...`로 로컬 저장 후 로컬 상대경로로 연결한다.

## 17) 기능 분해형 게임 기획서 고정 규칙 (재정의)
- 목적:
  - 기획서를 사업/BM 중심 문서가 아니라 개발 실행 문서로 고정한다.
- 문서 구조:
  - 메인 문서: `docs/plans/latest_plan.md`
  - 기능 문서: `docs/plans/features/systems/*.md`, `docs/plans/features/ui/*.md`
- 메인 문서 필수 범위:
  - 게임 전체 흐름(mermaid 1개 이상)
  - 화면 흐름(mermaid 1개 이상)
  - 기능 분해 인덱스(기능 ID, 기능명, 문서 경로)
  - 구현 우선순위(스프린트 단위)
- 기능 문서 필수 템플릿:
  - 목표
  - 입력/출력
  ...
  - 검수 기준
- UI 문서 필수 규칙:
  - 화면별로 개별 파일을 만든다(한 파일에 UI 전체를 몰아쓰지 않는다).
  - 각 UI 파일에 컴포넌트 ID 표를 포함한다.
  - 각 UI 파일에 화면 플로우를 포함한다.
- 금지 규칙:
  - 메인 문서를 사업계획서 형식(BM/예산 중심)으로 작성 금지.
  - 기능 상세 없이 선언형 문장만 나열 금지.
