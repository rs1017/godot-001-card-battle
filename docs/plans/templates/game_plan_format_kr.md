...
### 7-1.6 샘플-2(v2) 적용 체크리스트
| 체크 항목 | 완료 여부(Y/N) | 비고 |
|---|---|---|
| 시나리오 -> 맵 -> 지역 -> 오브젝트 -> 범례 -> 백그라운드 -> 랜드마크 -> UI 순서 준수 |  |  |
| 2D 평면도/맵 링크/범례를 로컬 이미지로 연결 (`docs/plans/images/...`) |  |  |
| 맵 연결관계 다이어그램(mermaid) 포함 |  |  |
| 맵 파라미터 CSV 생성 (`docs/plans/data/map_*.csv`) |  |  |
| 지역별 위험/보상/상호작용 오브젝트 표 작성 |  |  |
| 캐릭터 선택 UI/캐릭터 생성 UI 화면 명세 완료 |  |  |
| 외부 웹 이미지 URL 직접 링크 0건 확인 |  |  |
